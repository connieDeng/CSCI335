...
class AvlTree
{
  public:
    ...
    double averageDepth()
    {
        return sumNodeDepth(root,0)/double(numOfNodes(root));
    }

    // Returns the number recursive calls to find n
    int totalRecursion(const string& n)
    {
        Comparable temp{n};
        return 1 + totalRecursion(root,temp);
    }
...
*****************************************************/
    ...
    double sumNodeDepth(AvlNode* t, int depth)
    {
        if (t == nullptr)
            ...
            return depth + sumNodeDepth(t->left,depth+1) + sumNodeDepth(t->right,depth+1);
    }

    //Returns the amount of recursive calls to find n
    int totalRecursion(AvlNode* t, const Comparable& n)
    {
        if(t == nullptr)
            return 0;
        else{
            if (t->element < n)
                return 1 + totalRecursion(t->right,n);
            else if (n < t->element)
                return 1 + totalRecursion(t->left,n);
            else
                return 0;            
        }
    }    
